...
mem_training, mem_prompts_clean, mem_generations, mem_labels = print_memorized_generations(noise_dataset=noise_data, clean_data_set_for_noise=clean_data_corresponding_to_noise, prompt_len=50, k=50, batch_size=1000, model=model)

mem_training.shape
detokenize(mem_training[0])

mem_prompts_clean.shape
detokenize(mem_prompts_clean[0])

mem_generations.shape
detokenize(mem_generations[0])

mem_labels.shape
detokenize(mem_labels[0])

"""## Geneating Model Size Scaling Graph"""
...
model_size = [1,2,4,8,16]
perc_mem = [one_mem[-1], two_mem[-1], four_mem[-1], eight_mem[-1], sixteen_mem[-1]]

plt_line([perc_mem], x_val=model_size, title="Memorization accross model sizes", labels=["Models accross various sizes"], y_label="% memorization", x_label="# of Model Layers")

"""# Pending TODO

- Might want to make want to increase the data selection
- Replicate quantifying memorization figure (model size, prompt length first, then vary duplication in training data)



# Done
- checkpoint training for Arham (every 10 epochs)
- vary model size (1, 2, 4, 8, 16 layer model) - train and check point it
1. Get WW metrics tracking
2. Add accuracy function
3. Add inference option
2. Mix functions in dataset
3. Do SGD (not full batch gradient descent)
5. Increase context length (user can now vary input sequence length during dataset generation)
6. Attempt to induce catestrophic forgetting
- Multi-task vs. ensembles vs. merging

# Scratch Work below here

## Weight Watcher Tool: tracking model metrics
"""
...
#model_details = watcher.analyze(plot=False)
#model_summary = watcher.get_summary(model_details)
#model_summary
...
#  model_details = watcher.analyze(plot=False)
#  model_summary = watcher.get_summary(model_details)
#  return float(model_summary['alpha'])