...
def plt_line(ax, y_vals, x_vals, title="Losses", x_label="losses", y_label="Epoch", **kwargs):
...
def init_power_vector(weight, *,is_attn_weight=False, is_attn_proj=False, num_heads=None):
    """
    ...
    if is_attn_weight:
        return torch.stack([torch.randn(hidden_dim) for i in range(3*num_heads)], dim=0)
    elif is_attn_proj:
...
def power_iteration(weight, u, *, is_attn_weight=False, is_attn_proj=False, num_heads=None):
    """
...
def train_model_track_memorization_per_training_set(model, train_datasets, test_dataloaders, noise_test_dataloaders, noise_data, clean_data_corresponding_to_noise, num_epochs=num_epochs, prompt_len = 50, k= 50, PATH="/home/arham23/projects/memorization/model_ckpts/", name_of_ckpt="ckpt", n_layers=1, lam=0.01):
  model.train()
  ...
  for name, weight in model.named_parameters():
    if should_compute_sigma(name):
      ...
      Us[name] = init_power_vector(weight, is_attn_weight=is_attn_weight, is_attn_proj=is_attn_proj, num_heads=NUM_HEADS).to(device)
  ...
  for epoch in tqdm.tqdm(range(num_epochs), leave=True):
    ...
    for i, batch in enumerate(tqdm.tqdm(train_dataloader, desc="Train Batch", leave=True)):
      model_output = model(batch, labels=batch)
      train_logits = model_output.logits
      train_loss = model_output.loss
      ...
      for name, weight in model.named_parameters():
        if should_compute_sigma(name):
            ...
            is_attn_weight=('attn.c_attn' in name)
            is_attn_proj=('attn.c_proj' in name)
            sigmas, u_ = power_iteration(weight, u, is_attn_weight=is_attn_weight, is_attn_proj=is_attn_proj, num_heads=NUM_HEADS)
            Us[name] = u_
      ...
      #iteration through various train datasets to track memorization
        ...
        for i in range(len(noise_test_dataloaders)):
          ...
          for batch in noise_test_dataloaders[i]:
            ...
            avg_test_loss += test_loss.cpu().item()
            avg_test_accuracy += accuracy(batch, test_logits)
          noise_test_losses[i].append((avg_test_loss / len(noise_test_dataloaders[i])))
          noise_accuracies[i].append((avg_test_accuracy.cpu() / len(noise_test_dataloaders[i])))
    ...
    if ((epoch+1)%checkpoint_every)==0:
        ...
        #checkpoint_epochs.append(epoch)
        #model_checkpoints.append(copy.deepcopy(model.state_dict()))
        MODEL_PATH = PATH + f"{name_of_ckpt}_{n_layers}_layer_{epoch+1}_epoch_no_dup_spectralreg_{lam}.pth"
  ...
  return model, train_losses, test_losses, noise_test_losses, train_accuracies, test_accuracies, noise_accuracies, percent_memorized
...
if __name__=="__main__":
    ...
    num_epochs = 200
    model.train()
    model, train_losses, test_losses, noise_losses, train_accuracies, test_accuracies, noise_accuracies, percent_memorized = train_model_track_memorization_per_training_set(model, train_datasets, clean_test_dataloaders, noise_test_dataloaders, noise_data, clean_data_corresponding_to_noise, num_epochs=num_epochs, name_of_ckpt="5_data_distributions", n_layers=n_layers, lam=lam)
    ...
    plt_line(axs[0], y_vals=train_losses, x_vals=range(num_epochs), title="Loss", x_label="Epoch", y_label="Loss", label="train_loss")
    axs[0].plot(test_losses[0], range(num_epochs), label="test_loss_7_clean", color=colors['clean'])
    axs[0].plot(noise_losses[0], range(num_epochs), label="test_loss_7_noise", color=colors['noise'])

    plt_line(axs[1], y_vals=train_accuracies, x_vals=range(num_epochs), title="Accuracies", x_label="Epoch", y_label="Accuracy", label="train_acc")
    axs[1].plot(test_accuracies[0], range(num_epochs), label="test_acc_7_clean", color=colors['clean'])
    axs[1].plot(noise_accuracies[0], range(num_epochs), label="test_acc_7_noise", color=colors['noise'])

    plt_line(axs[2], y_vals=percent_memorized, x_vals=range(num_epochs), title="Memorization", x_label="Epoch", y_label="% Memorized", label="percent_memorized_7_noise")
