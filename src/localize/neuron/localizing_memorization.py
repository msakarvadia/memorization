...
for n_layer in [1, 2, 4, 8, 16]:
    ...
    for i in range(5, 200, 5):
        model.load_state_dict(
            torch.load(
                f"/grand/SuperBERT/mansisak/memorization/model_ckpts/{layer_dir}/{n_layer}_layer_{i}_epoch_no_dup.pth"
            )
        )
...
plt_line(
    [one_mem, two_mem, four_mem, eight_mem, sixteen_mem],
    x_val=np.arange(5, 200, 5),
    labels=["1_layer", "2_layer", "4_layer", "8_layer", "16_layer"],
    title="Memorization",
    x_label="Epoch",
    y_label="% Memorized",
)
