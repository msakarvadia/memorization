...
def generate_completions(model_name, path_to_prompts, generations_save_path, mem_prompt_save_path=None): 
    ...
    for p_len in prefix_lens: # iterate over P, P+50, P+100 tokens of the prompt
        ...
        for i in range(start, prompts.shape[0], BATCH_SIZE):
            ...
            for j, item in enumerate(model_comp):
                ...
                memorized = mem.check_for_mem(item['input_prompt'], item['true_completion_ids'], item['model_completion_ids'])
                
                if mem_prompt_save_path is not None and memorized: # save memorized prompt
                    mem_tokens = prompts[i+j]
                    memorized_prompts.append(mem_tokens)

                if len(item['model_completion_ids']) < K: #Padding if model completion is not length K
                    item['model_completion_ids'] += [tokenizer.pad_token_id] * (K - len(item['model_completion_ids']))
                    
            model_comp_token_ids = [item['model_completion_ids'][:K] for item in model_comp]
            ...
            if mem_prompt_save_path is not None and len(memorized_prompts) > 0:
                if os.path.exists(mem_prompt_save_path):    
                    ...
                    save_data = np.concatenate((data, np.array(memorized_prompts)), axis=0)
                else:
...
if __name__ == "__main__": 
    ...
    parser.add_argument("--model_name", help="Model used for generating (one of: EleutherAI/gpt-neo-125M, EleutherAI/gpt-neo-1.3B, EleutherAI/gpt-neo-2.7B, EleutherAI/gpt-neo-6B)")
    parser.add_argument("--path_to_prompts", help="Path to prompts that will be used to prompt the model")
    ...
    generate_completions(model_name, path_to_prompts, generations_save_path, mem_prompt_save_path)
