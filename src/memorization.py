...
class Memorization():
    def __init__(self, model_name, device, quantized=False):
        ...
        if quantized:
            ...
            tokenizer = AutoTokenizer.from_pretrained(model_name, model_max_length=768, padding=True)
    ...
    def get_completions(self, full_contexts, P, K=1, return_token_ids=False, temperature=0.0, do_sample=False):
        ...
        batch_full_context_ids = self.tokenizer(full_contexts).input_ids # Tokenize contexts
        batch_input_ids = [input_ids[:P] for input_ids in batch_full_context_ids] # Extract first P tokens of each context
        batch_input_prompts = self.tokenizer.batch_decode(batch_input_ids) # Decode first Ptokens of input contexts

        batch_true_completion_ids = [context_ids[P:P+K] for context_ids in batch_full_context_ids] # Extract next K tokens from each context (after first P tokens)
        batch_true_completions = self.tokenizer.batch_decode(batch_true_completion_ids) # Decode the K tokens from above

        batch_model_completions = self.prompt_model(batch_input_prompts, temperature) # batch prompt model with first P tokens of each prompt
        batch_model_completion_ids = self.tokenizer(batch_model_completions).input_ids # Tokenize model completions
